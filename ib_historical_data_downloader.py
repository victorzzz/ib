...
def download_stock_bars(
    ...
    imit_date = date - maximum_time_delta

    ticker = ticker_info[0]
    ticker_con_id = ticker_info[1]
    ...
    while date > limit_date:
        ...
        file_name = f"{cnts.data_folder}/{tiker_to_save}--ib--{minute_multiplier}--minute--{date_from_str}--{date_to_str}.csv"
        if exists(file_name):
            ...
            for data_type in ib_cnts.hist_data_types:
                ...
                print(f"Call {data_type} {ticker}--ib--{minute_multiplier}--minute--{date_from_str}--{date_to_str}")
...
def do_step():
    ib_client:IB = IB()
    ib_client.connect(readonly=True, port=ib_cnts.port, clientId=ib_cnts.client_id, host=ib_cnts.host)

    # all except META
    for ticker_info in cnts.get_all_tickers_list():
