...
def add_technical_indicators(df:pd.DataFrame) -> pd.DataFrame:
    ...
    for period in (7, 14, 21, 28):
        df[f'MFI_MIDPOINT_{period}'] = ta.volume.money_flow_index(df['MIDPOINT_high'], df['MIDPOINT_low'], df['MIDPOINT_close'], df['TRADES_volume'])
        df[f'MFI_TRADES_average'] = ta.volume.money_flow_index(df['TRADES_high'], df['TRADES_low'], df['TRADES_average'], df['TRADES_volume'])
        # df['MFI_BID_low_ASK_high_MIDPOINT'] = ta.volume.money_flow_index(df['ASK_high'], df['BID_low'], df['MIDPOINT_close'], df['TRADES_volume'])
